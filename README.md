...
本项目为 CS599 课程大作业，完整报告和演示视频如下：

| 资源 | 文件 | 说明 |
|---|---|---|
| 大作业报告 | [CS599_大作业报告.pdf](CS599_大作业报告.pdf) | 详细介绍了项目的背景、系统架构、多 Agent 设计、检索策略、实验评估与结论。 |
| 演示视频 | [smart_recipe.mp4](smart_recipe.mp4) | 系统功能演示，展示文本问答、图片识别、多轮对话、偏好记忆、数据库查询等核心功能。 |

报告涵盖以下内容：
- 项目背景与研究动机：为什么需要一个结合结构化数据、知识图谱和 RAG 的多 Agent 菜谱问答系统。
- 系统架构设计：基于 LangGraph 的多 Agent 工作流，包括 Safety、Preference、Router、Recipe、Nutrition、SQL、Cypher、Fusion、Rerank、Answer 等 Agent 的分工与协作。
- 检索与融合策略：FAISS HNSW 向量召回 + BM25 关键词召回 + LLM HyDE 扩展 + Cross-Encoder 精排 + 多源候选融合。
- 数据存储方案：MySQL 结构化存储、Neo4j 知识图谱、Redis 会话记忆的协作。
- 实验评估：路由准确率、SQL/Cypher 查询正确率、RAG 检索质量、偏好记忆效果等多维度评估结果。
...
- Answer Guard：数据库/RAG/规则候选为空时，必须结合当前问题和 `session_id` 偏好调用 LLM 生成明确标注的通用参考推荐，不能只回复“没有结果”或要求用户更换筛选条件；LLM 不可用或输出触及过敏/不吃食材时，改用本地安全推荐模板。图片识别不确定时则自动降级为谨慎回答。
...
| Redis | Docker | 按 `session_id` 保存偏好和多轮会话；Redis 不可用时退回进程内存。 |
| Neo4j | Docker | 保存菜谱和食材、标签、目标之间的图关系。 |
| JSON/向量索引 | 本机项目目录 | RAG 原始数据和本地检索缓存。 |

当前 MySQL 只保留以下 5 张业务表：
...
`chat_turns`、`eval_runs`、`user_profiles`、`recipe_suitable_for` 和 `recipe_tags` 已移除，不会在初始化或服务重启时重新创建。会话偏好不写入 MySQL，而是由 Redis/内存按 `session_id` 隔离保存；标签、目标和约束关系仍保存在 Neo4j 图谱中。
...
根地址会自动跳转到对话界面。需要查看数据库时，点击左侧底部唯一的“数据库预览”按钮；预览页会读取 `.env` 里的 `MYSQL_*`、`REDIS_URL`、`NEO4J_*` 配置，左侧选择 MySQL 表、Redis key 或 Neo4j 节点标签，右侧预览数据。这个页面是只读预览，不会修改数据库。
...
```cmd
python scripts/start_server.py --host 127.0.0.1 --port 8000
```
...
- 对话页面：`http://127.0.0.1:8010/`（自动跳转到 `/ui/`）
- 数据库浏览页面：`http://127.0.0.1:8010/ui/database.html`
- 健康检查：`http://127.0.0.1:8010/health`

调试页面左侧会在当前浏览器的 `localStorage` 中保存对话列表和显示消息。每次新建对话都会生成独立 `session_id`；切换对话时，后端会按该 ID 读取对应的历史与逐步提取出的偏好，不需要预先填写偏好。点击任意对话右侧的“⋯”会展开操作菜单，再点击“删除”并确认即可删除该条记录。
...
```cmd
python scripts\rebuild_pdf1_pipeline.py --dry-run --ocr-max-pages 10 --output-prefix pdf1_smoke
```

确认 OCR、分块、质量报告和索引生成正常后，执行完整重建。下面这条命令会使用规则结构化；校验失败的菜谱只写入质量报告，不进入 MySQL/Neo4j：

```cmd
python scripts\rebuild_pdf1_pipeline.py
```

如果 `.env` 已配置可用的文本 LLM，可启用固定 JSON 结构化并保留缓存，随后再经过同一套规则校验：

```cmd
python scripts\rebuild_pdf1_pipeline.py --llm-structure
```

若要求任一 LLM 调用失败就终止整次重建，而不是退回规则结构化：

```cmd
python scripts\rebuild_pdf1_pipeline.py --llm-structure --require-llm
```
...
data\processed\1_recipe_quality.json
data\processed\1_recipe_llm_cache.json（启用 --llm-structure 时生成）

MySQL: document_indexes, document_chunks
MySQL: recipes, ingredients, recipe_ingredients
...
注意：`document_indexes` / `document_chunks` 保存 PDF chunk 正文、metadata 和本地 FAISS 路径；`recipes` 等结构化表保存“一道菜一行”的规范菜谱；Neo4j 保存菜谱、食材、标签、目标之间的图关系。统一脚本会把 OCR 原料字段截断在制作动作之前，过滤“切丝、洗净、倒入土豆丝”等步骤短语，并在写库前校验菜名、食材和步骤。提交前还会统一补齐 `category`、`cooking_time_minutes`、`difficulty`、`calories_per_100g`、`protein_g_per_100g` 和 `fat_g_per_100g`，再把同一份规范数据同步到 MySQL 与 Neo4j。营养值没有原文依据时会标记 `nutrition_estimated=true`。时间只持久化分钟数，不再同时保存容易冲突的时间文本。未通过的记录保存在 `*_recipe_quality.json`，不会直接进入数据库。
...
停止 FastAPI：在运行启动器的终端按 `Ctrl+C`。启动器默认在连续 5 分钟没有收到任何 HTTP 请求后自动关闭；再次运行启动命令时，也会先关闭上一份由启动器管理的 SmartRecipe 服务。若要关闭空闲超时，可加 `--idle-timeout 0`。
