...
- 多 Agent 工作流：基于 LangGraph 串联 Safety、Preference、Router、Recipe、Nutrition、SQL、Cypher、Fusion、Vision、Rerank、Tool、Answer 等 Agent。
- Tool Agent 工具调用：内置菜谱搜索、偏好查询、文档检索、食材过滤、购物清单、周菜单规划、SQL/Neo4j 查询 7 个工具，Tool Agent 优先通过 LLM 规划选择工具，LLM 不可用时回退到关键词规则匹配。
...
- Query Rewrite：推断中文查询意图（做法/推荐/食材/营养），自动补充同义词、去除停用词，提升检索召回率。
...
│  │  ├─ tool_agent.py           # LLM/规则工具规划与执行
│  │  └─ support_agents.py       # Safety、General 等辅助 Agent
│  ├─ tools/
│  │  ├─ base.py                 # Tool 协议与 ToolResult 定义
│  │  ├─ registry.py             # 工具注册与 build_default_tool_registry
│  │  ├─ recipe_tools.py         # 菜谱搜索、偏好查询工具
│  │  ├─ database_tools.py       # MySQL/Neo4j 只读查询工具
│  │  ├─ document_tools.py       # 文档 FAISS 检索工具
│  │  └─ planning_tools.py       # 购物清单、周菜单、食材过滤工具
│  ├─ services/
│  │  ├─ query_boundary_guard.py # 输入规范化、边界检测和风险分类
│  │  ├─ query_understanding.py  # 领域纠错候选生成和关键约束保护
│  │  ├─ query_rewrite.py        # 中文查询意图推断与同义词扩展
...
│  │  ├─ hyde.py                 # LLM HyDE 假设文档生成，仅用于向量召回
...
│  │  ├─ database_browser.py     # MySQL/Redis/Neo4j 只读预览
│  │  ├─ server_activity.py     # 请求心跳与空闲自动关停
...
├─ scripts/
│  ├─ start_server.py           # 管理式 Uvicorn 启动、心跳监控与空闲关停
...
用户文本/图片
  ...
  -> Preference Agent
  -> Router Agent
  -> Recipe / Nutrition / SQL / Cypher / Fusion / Vision / Tool / General Agent
  -> Rerank Agent
  -> Answer Agent
  -> Answer Guard
  -> 返回结果
...
文本请求进入 `/chat` 后，系统先做安全检查和偏好提取，再由 Router 判断用户意图。普通推荐走 Recipe + RAG；营养问题走 Nutrition；结构化统计走 SQL；关系型问题走 Cypher；需要执行购物清单、周菜单等复合操作时走 Tool Agent；复杂问题可走 Fusion，把多个来源的候选融合成统一结果。
...
| Tool Agent | LLM 规划或规则匹配选择工具，执行菜谱搜索、购物清单、周菜单等。 |
