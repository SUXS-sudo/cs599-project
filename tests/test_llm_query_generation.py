...
class FakeSQLStore:
    ...
    def read_query(self, sql, parameters=()):
        ...
        return [{"name": "番茄炒蛋", "category": "家常菜", "cooking_time_text": "15分钟", "difficulty": "简单", "calories": 220}]
...
def test_sql_agent_uses_valid_llm_query() -> None:
    llm = FakeLLM(
        '{"sql":"SELECT r.name, r.category, r.cooking_time_text, r.difficulty, r.calories FROM recipes r ORDER BY r.calories ASC","params":[],"title":"低热量菜谱"}'
    )
    store = FakeSQLStore()
    ...
    assert "ORDER BY r.calories ASC" in store.queries[0][0]
...
def test_sql_agent_does_not_cache_empty_rows_and_triggers_llm_fallback(monkeypatch) -> None:
    ...
    first = agent.run(make_state("推荐低脂晚餐"))
    second = agent.run(make_state("推荐低脂晚餐"))
